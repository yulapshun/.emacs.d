...
Flycheck-Annotate mode is enabled in all buffers where `(lambda nil (when (flycheck-may-enable-mode)
(flycheck-annotate-mode 1)))' would do it.
...
Flycheck-Lsp mode is enabled in all buffers where `(lambda nil (when (flycheck-lsp--available-command major-mode)
...
Flycheck-Eglot mode is enabled in all buffers where `(lambda nil (when (flycheck-eglot--available-p)
(flycheck-eglot-mode 1)))' would do it.
