...
`Compile-Angel-On-Load mode' mode.  If the prefix argument is
...
This is a global minor mode.  If called interactively, toggle the
`Compile-Angel-On-Save mode' mode.  If the prefix argument is
positive, enable the mode, and if it is zero or negative, disable
the mode.
...
`Compile-Angel-On-Save-Local mode' mode.  If the prefix argument
...
To check whether the minor mode is enabled in the current buffer,
evaluate `compile-angel-on-save-local-mode'.
